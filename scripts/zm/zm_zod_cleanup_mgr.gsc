...
private function cleanup_main()
{
	...
	while(true)
	{
		...
		foreach(var_dbc455d5, ai_enemy in a_ai_enemies)
		{
...
function do_cleanup_check(n_override_cleanup_dist)
{
	...
	if(!b_in_active_zone)
	{
		...
		foreach(var_99630e9d, player in level.activeplayers)
		{
...
private function delete_zombie_noone_looking()
{
	...
	foreach(var_4d4fe28d, player in level.players)
	{
...
{
	a_wait_locations = [];
	foreach(var_9aa33dd5, zone in a_zones)
	{
